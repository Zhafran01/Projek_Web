	<div class="row">
						...
						<div class="modal-body">
							<form method="POST" enctype="multipart/form-data" action="<?= base_url('admin/tambah_kandidat'); ?>">
								<div class="form-group">
									...
									<input type="text" name="nama" class="form-control" placeholder="Klik Nama Kandidat" required="required">
								</div>
								<div class="form-group">
									<label for="keterangan">Motto :</label>
									<input type="text" name="ket" class="form-control" placeholder="(Motto)" required="required">
								</div>
								<div class="form-group">
		...
		<div class="col-sm-12">
			<div class="tengah">
				<div class="col-sm-12">
					<h3 class="text-center">List Kandidat</h3>
				</div>
				<?php
				if (count($kandidat) > 0) {
					...
					<div class="col-sm-12">
						<div class="box box-solid">
							<div class="box-body">
								...
								<h3 class="text-center" style="color:red;">Tidak Ada Kandidat yang Ditemukan!</h3>
							</div>
						</div>
					</div>
				<?php } ?>
	...
	<div class="modal fade" id="edit">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button class="close" data-dismiss="modal">&times;</button>
					<h4 class="modal-title"><i class="fa fa-user"></i> Ubah Kandidat</h4>
				</div>
				<div class="modal-body">
					<form action="<?= base_url('admin/edit_kandidat') ?>" method="POST" enctype="multipart/form-data">
						...
						<div class="form-group">
							...
							<input type="text" name="nama" class="form-control" placeholder="Nama Kandidat">
						</div>
						<div class="form-group">
							<label for="Keterangan">Motto :</label>
							<input type="text" name="ket" class="form-control" placeholder="(Motto)">
						</div>
						<div class="form-group">
							<label for="Foto">Foto Kandidat :</label>
	...
	<script>
		$(document).ready(function() {
			$('.hapus').on('click', function() {
				...
				var h = confirm('Apakah Anda yakin ingin menghapus kandidat?');
				if (h) {
	...
	<?= ($this->session->flashdata('tambah')) ? '<script>Swal.fire("Sukses", "Kandidat berhasil ditambahkan", "success")</script>' : '' ?>
	<?= ($this->session->flashdata('hapus')) ? '<script>Swal.fire("Sukses", "Kandidat telah berhasil dihapus", "success")</script>' : '' ?>
