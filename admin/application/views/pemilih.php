				...
				<div class="box-body">
					<form id="formtambah">
						<div class="form-group">
							...
							<input type="text" name="nama" class="form-control" placeholder="Klik Nama">
						</div>
						<div class="form-group">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title"><i class="fa fa-user"></i> Ubah Pemilih</h4>
				</div>
	...
	<script>
				...
				success: function(data) {
					...
					for (i = 0; i < data.length; i++) {
						html += '<tr>' +
							...
							'<td><button class="btn btn-xs btn-flat btn-success edit" data="' + data[i].id_pemilih + '"><i class="fa fa-edit"></i> Ubah</button>&nbsp;&nbsp;<button class="btn btn-xs btn-flat btn-danger hapus" data="' + data[i].id_pemilih + '"><i class="fa fa-trash"></i> Hapus</button></td>' +
							'</tr>';
					}
					$('#showData').html(html);
				}
		...
		$('#showData').on('click', '.reset-pass', function(e) {
			...
			if (k) {
				$.ajax({
					...
					success: function(data) {
						Swal.fire('Sukses', 'Password berhasil di reset', 'success');
					}
				})
				return false;
			}
		...
		$('#showData').on('click', '.hapus', function() {
			var id = $(this).attr('data');
			var k = confirm('Apakah Anda yakin ingin menghapusnya ?');
			if (k) {
				$.ajax({
